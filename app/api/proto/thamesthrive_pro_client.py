...
from ThamesThrive.service.pro.auth import get_tpro_token
...
class ThamesThriveProClient(object):
    """
    ...
    def __init__(self, host, port=50000, secure=False):
        ...
        logger.info(f"ThamesThrive PRO set to {host}")
        if secure:
    ...
    async def get_available_services(self, query="", category=""):
        message = pb2.ServiceQuery(query=query, category=category, version=ThamesThrive.version.version)
