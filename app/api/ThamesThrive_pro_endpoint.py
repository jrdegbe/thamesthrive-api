...
from ThamesThrive.domain.pro_service_form_data import TProMicroserviceCredentials, ProService, ProMicroService
from ThamesThrive.service.plugin.domain.register import Plugin, MicroserviceConfig
from ThamesThrive.service.plugin.plugin_install import install_remote_plugin, install_plugin
...
from ThamesThrive.domain.sign_up_data import SignUpData, SignUpRecord
from app.api.proto.tracard_pro_client import ThamesThriveProClient
...
ThamesThrive_pro_client = ThamesThriveProClient(host=ThamesThrive.ThamesThrive_pro_host,
                                        port=ThamesThrive.ThamesThrive_pro_port,
                                        secure=False)
...
async def is_token_valid():
    ...
    token = ThamesThrive_pro_client.validate(token=result.token)

    return token is not None


@router.post("/tpro/sign_in", tags=["tpro"], include_in_schema=server.expose_gui_api)
async def ThamesThrive_pro_sign_in(credentials: Credentials):
    """
    Handles signing in to ThamesThrive PRO service
    """
    try:
        token, host = ThamesThrive_pro_client.sign_in(credentials.username, credentials.password)
        result = await pro_db.read_pro_service_endpoint()
        ...
        if result is None or result.token != token:
            ...
            if result.saved == 0:
                raise ConnectionError("Could not save ThamesThrive Pro data.")
...
async def ThamesThrive_pro_sign_up(sign_up_data: SignUpData):
    """
    Handles signing up to ThamesThrive PRO service
    """
    try:
        # todo save more contact data on Pro server
        token = ThamesThrive_pro_client.sign_up(sign_up_data.username, sign_up_data.password)
        sign_up_record = SignUpRecord(id='0', token=token)
        result = await pro_db.save_pro_service_endpoint(sign_up_record)

        if result.saved == 0:
            raise ConnectionError("Could not save ThamesThrive Pro endpoint.")
...
async def get_available_services(query: Optional[str] = "", category: Optional[str] = ""):
    """
    Returns available ThamesThrive PRO services
    """
    try:
        services = await ThamesThrive_pro_client.get_available_services(query, category)
...
async def get_available_services(module: str):
    ...
    """
    try:
        return await ThamesThrive_pro_client.get_plugin(module)
...
async def save_ThamesThrive_pro_resource(pro: ProService):
    """
    Adds new ThamesThrive PRO resource
    """
    result = {}
...
async def save_ThamesThrive_pro_microservice(pro: ProMicroService):
    """
    Adds new ThamesThrive PRO resource
    """
